...
class ParrotInterface {
    ...
    async transferToken(keyringPair, to, tokenId, amount) {
        const prc20TransferTx = await this.api.tx.prc20.transfer(
            to,
            tokenId,
            amount
        );
    ...
    async tokenTransferFrom(keyringPair, from, to, tokenId, amount) {
        const transferFromTx = await this.api.tx.prc20.transferFrom(
            from,
            to,
            tokenId,
            amount
    ...
    async createSignedOffer(offer, signature, signer) {
        const signedOffer = await this.api.createType('SignedOffer', {
            offer,
            signature,
            signer,
        });
    ...
    async createDelegatedTransferDetails(senderAddress, receiverAddress, amount) {
        ...
        const dtd = await this.api.createType('DelegatedTransferDetails', {
            amount,
            to: receiverAddress,
            nonce,
        });
    ...
    async createSignedDtd(dtd, signature, signer) {
        const signedDtd = await this.api.createType(
            'SignedDelegatedTransferDetails',
            { transfer: dtd, signature, signer }
        );
        return signedDtd;
    }
}
export default ParrotInterface;
